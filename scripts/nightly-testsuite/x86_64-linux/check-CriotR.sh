...
function run_tests ()
{
  ...
  make -C ../rtl clean all FPC=fpc OPT="$OPT" > rtl-fpcsrc-$TEST_FPC.log 2>&1
  make -C ../packages clean fpmake  FPC=fpc OPT="$OPT" > packages-fpmake-$TEST_FPC.log 2>&1
  make -j 5 full TEST_FPC=$TEST_FULL_FPC TEST_OPT="$TEST_OPT" FPC=fpc OPT="$OPT" > full-$TEST_FPC.log 2>&1
...
function checkres ()
{
  ...
  make_line='make -C ../compiler distclean cycle OPT="-n $COMMON_OPT" CPU_TARGET=x86_64'
  echo "Starting $make_line"
  make -C ../compiler distclean cycle OPT="-n $COMMON_OPT" CPU_TARGET=x86_64 > ppcx64-cycle.log 2>&1
  res=$?
  checkres $make_line

  make_line='make -C ../compiler installsymlink OPT="-n $COMMON_OPT" FPC=`pwd`/../compiler/ppcx64'
  echo "Starting $make_line"
  make -C ../compiler installsymlink OPT="-n $COMMON_OPT" FPC=`pwd`/../compiler/ppcx64 > ppcx64-installsymlink.log 2>&1
  res=$?
  checkres $make_line

  make_line='make -C ../compiler distclean cycle OPT="-n $COMMON_OPT" LOCALOPT="$COMMON_OPT -CriotR" FPC=ppcx64'
  echo "Starting $make_line"
  make -C ../compiler distclean cycle OPT="-n $COMMON_OPT" LOCALOPT="$COMMON_OPT -CriotR" FPC=ppcx64 > ppcx64-cycle-CriotR.log 2>&1
  res=$?
  checkres $make_line
  ...
  make_line='make -C ../compiler distclean cycle OPT="$COMMON_OPT" CPU_TARGET=i386'
  echo "Starting $make_line"
  make -C ../compiler distclean cycle OPT="-n $COMMON_OPT" CPU_TARGET=i386 > ppc386-cycle.log 2>&1
  res=$?
  checkres $make_line

  make_line='make -C ../compiler installsymlink OPT="$COMMON_OPT"  FPC=`pwd`/../compiler/ppc386'
  echo "Starting $make_line"
  make -C ../compiler installsymlink OPT="-n $COMMON_OPT"  FPC=`pwd`/../compiler/ppc386 > ppc386-installsymlink.log 2>&1
  res=$?
  checkres $make_line

  make_line='make -C ../compiler distclean cycle OPT="$COMMON_OPT" LOCALOPT="$COMMON_OPT -CriotR" FPC=ppc386'
  echo "Starting $make_line"
  make -C ../compiler distclean cycle OPT="-n $COMMON_OPT" LOCALOPT="$COMMON_OPT -CriotR" FPC=ppc386 > ppc386-cycle-CriotR.log 2>&1
  res=$?
  checkres $make_line
