...
SELECT (TU_FAILEDTOCOMPILE+TU_FAILEDTOFAIL+TU_FAILEDTORUN) as FAILS,DATE(TU_DATE) as DATE,TESTVERSION.TV_VERSION as VERSION,TESTOS.TO_NAME as OS,TESTCPU.TC_NAME as CPU,TU_SUBMITTER as TESTER,TU_MACHINE as MACHINE,TU_COMMENT as COMMENT, TIME(TU_DATE) as TIME, TU_ID, IFNULL((select CONCAT(COUNT(TR_TEST_FK),"|",GROUP_CONCAT(TR_TEST_FK)) from TESTRESULTS where TR_TESTRUN_FK=TU_ID AND TR_OK<>"+" AND TR_SKIP<>"+" GROUP BY TR_TESTRUN_FK),"0|") as FAILLIST
...
WHERE TU_DATE>=DATE_SUB(CURDATE(), INTERVAL 2 DAY)
